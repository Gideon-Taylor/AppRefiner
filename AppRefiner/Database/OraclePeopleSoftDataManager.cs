...
namespace AppRefiner.Database
{
    ...
    public class OraclePeopleSoftDataManager : IDataManager
    {
        ...
        /// directly; chunked by 900 to stay under Oracle's 1000-item IN-list limit.
        /// </summary>
            ...
            try
            {
                ...
                foreach (DataRow row in data.Rows)
                {
                    ...
                    if (IDataManager.TryMapStringToTargetType(defnType, out OpenTargetType targetType))
                    {
                        ...
                        if (targetType is OpenTargetType.NonClassPeopleCode)
                        {
                            target.DisplayTargetType = defnType;
                        }
...
ORDER BY DEFN_TYPE ASC, ID ASC, CASE WHEN :sort_by_date = 'Y' THEN LASTUPDDTTM END DESC";
  ...
  WHERE  CONTTYPE = 9
     ...
     AND (LENGTH(:id_search) > 0 OR LENGTH(:descr_search) > 0)",
        ...
        private const string NON_CLASS_PPC_QUERY = @"
-- Page PeopleCode
...
WHERE OBJECTID1 = 9 AND OBJECTID2 = 12
  AND (LENGTH(:id_search) = 0 OR UPPER(OBJECTVALUE1) LIKE UPPER(:id_search || '%') ESCAPE '\')
  AND (LENGTH(:descr_search) = 0 OR UPPER(OBJECTVALUE1 || '.Activate') LIKE UPPER('%' || :descr_search || '%') ESCAPE '\')
  AND (LENGTH(:id_search) > 0 OR LENGTH(:descr_search) > 0)
...
WHERE OBJECTID1 = 10 AND OBJECTID2 = 39 AND OBJECTID3 = 12
  AND (LENGTH(:id_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE3) LIKE UPPER(:id_search || '%') ESCAPE '\')
  AND (LENGTH(:descr_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE3) LIKE UPPER('%' || :descr_search || '%') ESCAPE '\')
  AND (LENGTH(:id_search) > 0 OR LENGTH(:descr_search) > 0)
...
WHERE OBJECTID1 = 10 AND OBJECTID2 = 39 AND OBJECTID3 = 1 AND OBJECTID4 = 12
  AND (LENGTH(:id_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE3) LIKE UPPER(:id_search || '%') ESCAPE '\')
  AND (LENGTH(:descr_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE3 || '.' || OBJECTVALUE4) LIKE UPPER('%' || :descr_search || '%') ESCAPE '\')
  AND (LENGTH(:id_search) > 0 OR LENGTH(:descr_search) > 0)
...
WHERE OBJECTID1 = 10 AND OBJECTID2 = 39 AND OBJECTID3 = 1 AND OBJECTID4 = 2 AND OBJECTID5 = 12
  AND (LENGTH(:id_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE3 || '.' || OBJECTVALUE4) LIKE UPPER(:id_search || '%') ESCAPE '\')
  AND (LENGTH(:descr_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE3 || '.' || OBJECTVALUE4 || '.' || OBJECTVALUE5) LIKE UPPER('%' || :descr_search || '%') ESCAPE '\')
  AND (LENGTH(:id_search) > 0 OR LENGTH(:descr_search) > 0)
...
WHERE OBJECTID1 = 1 AND OBJECTID2 = 2 AND OBJECTID3 = 12
  AND (LENGTH(:id_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2) LIKE UPPER(:id_search || '%') ESCAPE '\')
  AND (LENGTH(:descr_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE3) LIKE UPPER('%' || :descr_search || '%') ESCAPE '\')
  AND (LENGTH(:id_search) > 0 OR LENGTH(:descr_search) > 0)
...
WHERE OBJECTID1 = 66 AND OBJECTID2 = 77 AND OBJECTID3 = 39 AND OBJECTID4 = 20 
  ...
  AND (LENGTH(:id_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE6) LIKE UPPER(:id_search || '%') ESCAPE '\')
  AND (LENGTH(:descr_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2 || '.' || OBJECTVALUE3 || '.' || 
...
WHERE OBJECTID1 = 74
  AND (LENGTH(:id_search) = 0 OR UPPER(OBJECTVALUE1) LIKE UPPER(:id_search || '%') ESCAPE '\')
  AND (LENGTH(:descr_search) = 0 OR UPPER(OBJECTVALUE1 || '.' || OBJECTVALUE2) LIKE UPPER('%' || :descr_search || '%') ESCAPE '\')
  AND (LENGTH(:id_search) > 0 OR LENGTH(:descr_search) > 0)
...
WHERE (OBJECTID1 = 60 OR OBJECTID1 = 87)
  ...
  AND (LENGTH(:id_search) > 0 OR LENGTH(:descr_search) > 0)";
        ...
        public List<(PSCLASSID[] ObjectIds, string[] ObjectValues)> GetProgramObjectIds(string[] objectValues)
        {
            ...
            string sql = @"
                SELECT DISTINCT OBJECTID1, OBJECTID2, OBJECTID3, OBJECTID4, OBJECTID5, OBJECTID6, OBJECTID7,
        ...
        public PeopleCodeType GetFieldType(string fieldName)
        {
            ...
            try
            {
                ...
                var fieldType = (decimal)result.Rows[0][0];
        ...
        /// <summary>
        /// Gets all package paths for a given application class name using Oracle SQL
        /// </summary>
        public List<string> GetPackagesForClass(string className)
        {
            ...
            try
            {
                ...
                string sql = @"
                    ...
                    ORDER BY
                        TRUNC(B.LASTUPDDTTM) DESC NULLS LAST,
                        A.PACKAGEROOT ASC
                ";
