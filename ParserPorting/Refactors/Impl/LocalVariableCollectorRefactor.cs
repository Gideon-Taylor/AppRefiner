    ...
    public class LocalVariableCollectorRefactor : ScopedRefactor
    {
        ...
        protected override void OnEnterScope(ScopeInfo scope)
        {
            ...
            if (scope.ScopeNode?.SourceSpan.IsValid == true)
            {
                ...
                scopeInsertionPoints[scopeKey] = scope.ScopeNode.SourceSpan.Start.Index;
            }
        }
        ...
        /// </summary>
        private string GetScopeKey(ScopeInfo scope)
        {
            if (scope.ScopeNode?.SourceSpan.IsValid == true)
            {
                return $"{scope.Type}_{scope.ScopeNode.SourceSpan.Start.Index}";
            }
            return $"{scope.Type}_unknown";
        }
        ...
        private void CollectVariablesInScope(string scopeKey, List<VariableInfo> variables)
        {
            ...
            var sortedVariables = variables.OrderByDescending(v => v.SourceSpan.Start.Index).ToList();
            foreach (var variable in sortedVariables)
            {
                DeleteNode(variable.Node, $"Remove original declaration of {variable.Name}");
            }
